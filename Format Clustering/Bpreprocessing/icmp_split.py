...
icmp_data = pd.read_csv('../Aextract+tag/extract-方便ET-BERT/icmp/icmp_traffic.csv')
hex_data = icmp_data['Hex Data']

clip_data_split = hex_data.apply(lambda x: ' '.join([x[i:i+2] for i in range(0, len(x), 2)]))

if len(clip_data_split) != len(icmp_data):
    raise ValueError("Lengths of clip_data_split and traindata do not match!")

icmp_data['Hex Data'] = clip_data_split

data_list = [icmp_data]
file_names = ["icmp_data"]  # 文件名列表
...
for data, file_name in zip(data_list, file_names):
    ...
    # output_path = os.path.join('../CET-BERT-main/datasets/icmp/', f'train{file_name}.tsv')
    traindata.to_csv('../CET-BERT-main/datasets/icmp/train'+ file_name+ '.tsv', sep='\t', index=False)
    ...
    valdata.to_csv('../CET-BERT-main/datasets/icmp/val'+ file_name +'.tsv', sep='\t', index=False)
    ...
    testdata.to_csv('../CET-BERT-main/datasets/icmp/test'+ file_name +'.tsv', sep='\t', index=False)
