...
nbns_data = pd.read_csv('../Aextract+tag/extract-方便ET-BERT/nbns/nbns_traffic.csv')
hex_data = nbns_data['Hex Data']

clip_data_split = hex_data.apply(lambda x: ' '.join([x[i:i+2] for i in range(0, len(x), 2)]))

if len(clip_data_split) != len(nbns_data):
    raise ValueError("Lengths of clip_data_split and traindata do not match!")

nbns_data['Hex Data'] = clip_data_split

data_list = [nbns_data]
file_names = ["nbns_data"]  # 文件名列表
...
for data, file_name in zip(data_list, file_names):
    ...
    # output_path = os.path.join('../CET-BERT-main/datasets/nbns/', f'train{file_name}.tsv')
    traindata.to_csv('../CET-BERT-main/datasets/nbns/train'+ file_name+ '.tsv', sep='\t', index=False)
    ...
    valdata.to_csv('../CET-BERT-main/datasets/nbns/val'+ file_name +'.tsv', sep='\t', index=False)
    ...
    testdata.to_csv('../CET-BERT-main/datasets/nbns/test'+ file_name +'.tsv', sep='\t', index=False)
