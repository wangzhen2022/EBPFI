...
ntp_data = pd.read_csv('../Aextract+tag/extract-方便ET-BERT/ntp/ntp_traffic.csv')
hex_data = ntp_data['Hex Data']

clip_data_split = hex_data.apply(lambda x: ' '.join([x[i:i+2] for i in range(0, len(x), 2)]))

if len(clip_data_split) != len(ntp_data):
    raise ValueError("Lengths of clip_data_split and traindata do not match!")

ntp_data['Hex Data'] = clip_data_split

data_list = [ntp_data]
file_names = ["ntp_data"]  # 文件名列表
...
for data, file_name in zip(data_list, file_names):
    ...
    # output_path = os.path.join('../CET-BERT-main/datasets/ntp/', f'train{file_name}.tsv')
    traindata.to_csv('../CET-BERT-main/datasets/ntp/train'+ file_name+ '.tsv', sep='\t', index=False)
    ...
    valdata.to_csv('../CET-BERT-main/datasets/ntp/val'+ file_name +'.tsv', sep='\t', index=False)
    ...
    testdata.to_csv('../CET-BERT-main/datasets/ntp/test'+ file_name +'.tsv', sep='\t', index=False)
